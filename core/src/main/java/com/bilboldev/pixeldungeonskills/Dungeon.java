...
import com.bilboldev.pixeldungeonskills.levels.CavesBossLevel;
import com.bilboldev.pixeldungeonskills.levels.CavesLevel;
...
import com.bilboldev.pixeldungeonskills.levels.PrisonBossLevel;
...
public class Dungeon {
	...
	@SuppressWarnings("deprecation")
	public static void switchLevel( final Level level, final int pos, final boolean restore) {
		...
		if(hero.hiredMerc != null) {
			hero.checkMerc = true;
			if (restore && depth != ColdGirl.FROST_DEPTH && depth != 0) {
				Actor.add(hero.hiredMerc);
				try {
					List<Mob> mercs = new ArrayList<>();
					for (Mob m : level.mobs) {
						if (m instanceof HiredMerc) {
							mercs.add(m);
						}
					}
					int size = mercs.size();
		...
		hero.pos = pos != -1 && pos != 0 ? pos : level.exit;
		if (hero.hiredMerc != null && (hero.hiredMerc.pos == 0 || hero.hiredMerc.pos == -1)) {
			hero.hiredMerc.pos = level.exit;
		}
