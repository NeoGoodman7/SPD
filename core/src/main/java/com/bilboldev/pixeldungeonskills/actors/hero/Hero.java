...
public class Hero extends Char {
    ...
    private static final String MERC_POS = "merchpos";
	...
	public void restoreFromBundle( Bundle bundle ) {
        ...
        if(tmp != null && tmp != "")
        {
            try
            {
                ...
                hiredMerc.pos = bundle.getInt(MERC_POS);
            }
            catch (Exception ex)
            {
        ...
        else
        {
			...
			heroSkills.lastUsed = skillTree.getActiveSkill();
	...
	public float speed() {
		...
		if (aEnc > 0) {
			...
			boolean sprint = ((HeroSprite)sprite).sprint( subClass == HeroSubClass.FREERUNNER && !isStarving() );
			return sprint ? 1.6f * speed : speed;
	...
	public boolean act() {
		...
		if (curAction == null) {
			...
			ready = false;
			// DeviceCompat.log("Actor", "hero ready false. frame->" + getFrame() + " curAction:" + curAction);
			// dumpStack();
	...
	public void busy() {
		ready = false;
		// DeviceCompat.log("Actor", "Hero->busy. frame->" + getFrame());
		// dumpStack();
	}
	...
	protected void ready() {
		...
		curAction = null;
		ready = true;
		// DeviceCompat.log("Actor", "Hero->ready. frame->" + getFrame());
		// dumpStack();
		GameScene.ready();
	}
	...
	private boolean actMove( HeroAction.Move action ) {
		// DeviceCompat.log("Scene", "move. deltaTime:" + Game.getDeltaTime() + " timeScale:" + Game.timeScale);
	...
	public boolean handle( int cell ) {
		...
		if (Dungeon.level.map[cell] == Terrain.ALCHEMY && cell != pos) {
			...
			} else if (ch instanceof ColdGirl && (((ColdGirl) ch).state instanceof ColdGirl.ColdGirlAI &&
					((((ColdGirl.ColdGirlAI)((ColdGirl) ch).state).aiStatus == ColdGirl.PASSIVE) || (((ColdGirl.ColdGirlAI)((ColdGirl) ch).state).aiStatus == ColdGirl.DONE_MODE)))) {
		...
		boolean result = act();
		// DeviceCompat.log("Scene", "act cost:" + (System.currentTimeMillis() - start));
		return result;
	}
