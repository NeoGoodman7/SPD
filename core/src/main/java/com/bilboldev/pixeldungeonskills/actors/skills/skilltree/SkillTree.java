...
public abstract class SkillTree {
    ...
    public Skill getActiveSkill() {
        for(Skill skill1 : skillDictionary.values()){
            if(skill1 instanceof ActiveSkill && skill1.active){
                return skill1;
            }
        }
        return null;
    ...
    public void restoreSkillsFromBundle( Bundle bundle ) {
        for(Skill skill1 : skillDictionary.values()){
            try
            {
                ...
                try
                {
                    skill1.availableAfter = bundle.getFloat("SKILL_3.0_COOLDOWN_" + " " + skill1.getClass().getName());
                }
                catch (Exception e){

                }
                skill1.active = bundle.getBoolean("SKILL_3.0_ACTIVE_" + " " + skill1.getClass().getName());
            }
            catch (Exception e){
    ...
    public void storeInBundle( Bundle bundle )
    {
        for(Skill skill1 : skillDictionary.values()){
            try
            {
                ...
                bundle.put( "SKILL_3.0_ACTIVE_" + " " + skill1.getClass().getName(), skill1.active);
            }
            catch (Exception e){
