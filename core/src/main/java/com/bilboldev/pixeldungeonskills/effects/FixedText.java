...
public class FixedText extends BitmapText {
	...
	private static SparseArray<ArrayList<FixedText>> stacks = new SparseArray<ArrayList<FixedText>>();
	...
	public static void show( float x, float y, int key, String text, int color ) {
		FixedText txt = GameScene.fixedStatus();
		txt.reset( x,  y,  text, color );
		push( txt, key );
	}
	
	private static void push( FixedText txt, int key ) {
		
		txt.key = key;
		
		ArrayList<FixedText> stack = stacks.get( key );
		if (stack == null) {
			stack = new ArrayList<FixedText>();
		...
		if (stack.size() > 0) {
			...
			while (aboveIndex >= 0) {
				FixedText above = stack.get( aboveIndex );
